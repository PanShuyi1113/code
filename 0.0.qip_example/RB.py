...
from qutip_qip.pulse import *

w0,w1,wc=4.16,5,4
yita0,yita1,yitac=-0.220,-0.090,-0.210
h0=w0*num(3)+yita0*0.5*(num(3)**2-num(3))
h1=w1*num(3)+yita1*0.5*(num(3)**2-num(3))
hc=yitac*0.5*(num(3)**2-num(3))+wc*num(3)
dwc=num(3)

mp=Processor(num_qubits=3,dims=[3,3,3],t1=0,t2=0)
mp.add_drift(h0,0)
mp.add_drift(h1,1)
mp.add_drift(hc,2)
mp.add_control(dwc,2,'dwc')

tlist=np.linspace(-50,50,101)

mp.get_qobjevo()
pulse=Pulse(dwc,2,coeff=False)
